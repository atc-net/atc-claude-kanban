...
$pngB64 = ''
...
Write-Jsonl (Join-Path $projectsRoot "acme\$s1.jsonl") @(
    ...
    # A ReportFindings tool call (rendered as ranked verdict/outcome-badged cards).
    [ordered]@{ type = 'assistant'; timestamp = (Stamp 33); message = [ordered]@{ role = 'assistant'; model = $opus; content = @([ordered]@{ type = 'text'; text = 'Here is the ranked findings report.' }, [ordered]@{ type = 'tool_use'; id = 'tu_findings'; name = 'ReportFindings'; input = [ordered]@{ level = 'high'; findings = @([ordered]@{ file = 'src/Order/OrderService.cs'; line = 142; summary = 'Null dereference when the cart is empty'; failure_scenario = 'CalculateTotal(new Cart()) throws NullReferenceException before returning 0'; verdict = 'CONFIRMED'; category = 'correctness' }, [ordered]@{ file = 'src/Payment/StripeClient.cs'; line = 88; summary = 'Retry loop has no backoff, can hammer the API'; failure_scenario = 'On a 5xx burst the client retries 50x in <1s and trips the rate limiter'; verdict = 'CONFIRMED'; outcome = 'fixed' }, [ordered]@{ file = 'src/Catalog/SearchIndexer.cs'; line = 210; summary = 'Redundant allocation inside a hot loop'; failure_scenario = 'Indexing 100k items allocates a new comparer each iteration'; verdict = 'PLAUSIBLE'; outcome = 'no_change_needed' }) } }); usage = (Usage 800 260 1200 142000) } },
    [ordered]@{ type = 'user'; timestamp = (Stamp 33); message = [ordered]@{ role = 'user'; content = @([ordered]@{ type = 'tool_result'; tool_use_id = 'tu_findings'; content = 'Reported 3 findings (2 confirmed, 1 plausible).' }) } },
    # A Read of an image file — the tool_result carries the image as base64 (rendered as a chip + preview).
    [ordered]@{ type = 'assistant'; timestamp = (Stamp 33); message = [ordered]@{ role = 'assistant'; model = $opus; content = @([ordered]@{ type = 'tool_use'; id = 'tu_readimg'; name = 'Read'; input = [ordered]@{ file_path = "$acmeCwd\docs\checkout-error.png" } }); usage = (Usage 300 80 500 143000) } },
    [ordered]@{ type = 'user'; timestamp = (Stamp 33); message = [ordered]@{ role = 'user'; content = @([ordered]@{ type = 'tool_result'; tool_use_id = 'tu_readimg'; content = @([ordered]@{ type = 'image'; source = [ordered]@{ type = 'base64'; media_type = 'image/png'; data = $pngB64 } }) }) } },
